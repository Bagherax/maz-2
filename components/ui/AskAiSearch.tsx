...
const AskAiSearch: React.FC = () => {
    ...
    const handleSearch = () => {
        const finalQuery = suggestion ? query + suggestion : query;
        if (!finalQuery.trim() || loading) return;

        updateQuery(finalQuery);
        executeQueryNow(finalQuery);
    };
        ...
        <div className="ask-ai-wrapper" ref={wrapperRef}>
            <div className={`ai-input-container ${isPanelOpen ? 'focused' : ''}`}>
                ...
                <div className="floating-dots">
                 ...
                 {/* NEW: Fake placeholder for multi-color text */}
                {!query && !suggestion && (
                    <div className="ai-input ai-placeholder" aria-hidden="true">
                        <span>Ask </span>
                        <span className="text-accent font-semibold">MAZ</span>
                        <span> AI</span>
                    </div>
                )}

                 {/* Suggestion Input (Bottom Layer) - displays the light blue suggestion text */}
                 <input
                    readOnly
                    tabIndex={-1}
                    value={suggestion ? query + suggestion : ''}
                    className="ai-input ai-suggestion-input"
                 />
                {/* User Input (Top Layer) - captures user typing */}
                <input
                    className="ai-input"
                    type="text"
                    value={query}
