...
const findOrCreateConversation = (
    ...
    if (!senderConvo) {
        senderConvo = { id: convoId, participantIds: [currentUser.id, receiver.id], participantDetails, messages: [] };
        senderConvos.push(senderConvo);
    }
    if (!receiverConvo) {
        receiverConvo = { id: convoId, participantIds: [currentUser.id, receiver.id], participantDetails, messages: [] };
...
export const startConversationWithSeller = async (seller: User): Promise<string> => {
    await simulateDelay(200);
    const currentUser = await getCurrentUser();
    if (!currentUser) throw new Error("Authentication required.");
    if (currentUser.id === seller.id) throw new Error("Cannot start a conversation with yourself.");

    const { convoId } = findOrCreateConversation(currentUser, seller);
    return convoId;
};
